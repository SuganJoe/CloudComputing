...
callservice() {
  ...
  do
   ...
   # echo $inputnum 
    ...
    output=`curl "https://networkio.azurewebsites.net/api/HttpTriggerCSharp1?code=POAkHV6n2tghAzhro5bygPWpH6drScdkmHNHtEUA6DhMMe7DIFgljQ==" >> /dev/null`
...
#########################################################################################################################################################
parallel --no-notice -j $threads -k callservice {1} {2} {"#"} ::: "${arpt[@]}"
#exit